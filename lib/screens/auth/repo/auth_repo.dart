  ...
  Future<Map<String, dynamic>> register({
    ...
    try {
      ...
      if (kDebugMode) {
        ...
        debugPrint('Confirm Password: ${confirmPassword.isNotEmpty ? '***SET***' : 'EMPTY'}');

        // File checks
        debugPrint('Driver License File Exists: ${await driverLicenseFile.exists()}');
        debugPrint('Driver License File Path: ${driverLicenseFile.path}');
        debugPrint('Driver License File Size: ${await driverLicenseFile.length()} bytes');

        debugPrint('Vehicle Registration File Exists: ${await vehicleRegistrationFile.exists()}');
        debugPrint('Vehicle Registration File Path: ${vehicleRegistrationFile.path}');
        debugPrint('Vehicle Registration File Size: ${await vehicleRegistrationFile.length()} bytes');
        debugPrint('=== REGISTRATION DATA END ===');
      }

      // Validate files exist before uploading
      if (!await driverLicenseFile.exists()) {
        throw Exception('Driver license file does not exist at path: ${driverLicenseFile.path}');
      }
      if (!await vehicleRegistrationFile.exists()) {
        throw Exception('Vehicle registration file does not exist at path: ${vehicleRegistrationFile.path}');
      }

      var data = FormData.fromMap({
        'driver_license': await MultipartFile.fromFile(
          driverLicenseFile.path,
          filename: driverLicenseFile.path.split(Platform.isAndroid ? '/' : '/').last,
        ),
        'vehicle_registration': await MultipartFile.fromFile(
          vehicleRegistrationFile.path,
          filename: vehicleRegistrationFile.path.split(Platform.isAndroid ? '/' : '/').last,
        ),
        'email': email,
        'mobile': mobile,
        'password': password,
      ...
      if (kDebugMode) {
        ...
        debugPrint('Total fields in FormData: ${data.fields.length + data.files.length}');
        debugPrint('Fields: ${data.fields.map((e) => '${e.key}: ${e.value}').join(' | ')}');
        debugPrint('Files: ${data.files.map((e) => '${e.key}: ${e.value.filename}').join(' | ')}');
      }
