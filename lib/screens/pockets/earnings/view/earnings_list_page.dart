...
class _EarningsListPageState extends State<EarningsListPage> {
  ...
  String _getAverageDailyEarnings(EarningsResponse response) {
    if (response.data?.earnings == null || response.data!.earnings!.isEmpty) {
      return '0';
    }

    final totalEarnings = double.tryParse(_getTotalEarnings(response)) ?? 0;
    final daysCount = _selectedDateRange == 'last_7_days' ? 7 : 30;
    final average = totalEarnings / daysCount;

    return average.toStringAsFixed(0);
  }

  String _getBestDayEarnings(EarningsResponse response) {
    if (response.data?.earnings == null || response.data!.earnings!.isEmpty) {
      return '0';
    }
    ...
    return maxEarnings.toStringAsFixed(0);
  }
